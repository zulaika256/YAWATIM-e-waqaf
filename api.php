...
// Get raw input if JSON is sent; for multipart/form-data (file uploads), use $_POST
$input = [];
    ...
    case 'add_donation':
        ...
        if (!empty($_FILES['attachment_image']) && $_FILES['attachment_image']['error'] === UPLOAD_ERR_OK) {
            ...
            if (!in_array($file_type, $allowed_types)) {
                json_resp('error', 'Invalid receipt image format. Only JPG, PNG, GIF, or WEBP are allowed.');
            }
            if ($file_size > 5 * 1024 * 1024) { // 5MB max
                json_resp('error', 'Receipt image is too large. Maximum size is 5MB.');
            }
            ...
            if (!move_uploaded_file($file_tmp, $dest)) {
                json_resp('error', 'Failed to save receipt image. Please try again.');
            }

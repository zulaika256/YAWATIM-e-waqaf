...
try {
    ...
    if ($role === 'admin') {
        ...
        // Wakalah partner â€” filter by their own channel only
        $sql = "SELECT d.*, w.name as collector_name, w.branch_name, w.type as collector_type 
                FROM donations d
                LEFT JOIN wakalah w ON d.wakalah_id = w.id
        ...
        <table class="table-yawatim" id="donations-table">
            ...
            <tbody>
                <?php if (count($donations) === 0): ?>
                    <?php
                        $colcount = 5; // Date, Month, Donor Details, Attachment, Amount
                        if ($role !== 'wakalah_individual') $colcount += 2; // state + booth
                        if ($role === 'admin') $colcount += 1; // wakalah
                    ?>
                    <tr>
                    ...
                    <?php foreach ($donations as $don): ?>
                        <tr>
                            ...
                            <td>
                                ...
                                <?php else: ?>
                                    <span style="color: var(--light-neutral); font-size: 0.8rem;">â€”</span>
                                <?php endif; ?>
                            </td>
                            <?php if ($role !== 'wakalah_individual'): ?>
                                <td>
...
<div class="modal-overlay" id="modal-add-donation">
    <div class="modal-container">
        ...
        <form id="form-add-donation" enctype="multipart/form-data">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="donor_name" id="don_donor_name" class="form-input" placeholder='e.g. Hajah Aminah â€” enter "-" for anonymous'>
                </div>
                ...
                <div class="form-group" id="don-receipt-group">
                    <label class="form-label" for="don_attachment_image">
                        ...
                        Attachment <span style="font-size: 0.75rem; color: var(--light-neutral);">(Optional â€” JPG, PNG, WEBP, max 5MB)</span>
                    </label>
...
document.addEventListener('DOMContentLoaded', () => {
    // ----- Donation form: use FormData to support file upload -----
    const donForm   = document.getElementById('form-add-donation');
    const submitBtn = document.getElementById('btn-record-donation');

    if (donForm && submitBtn) {
        donForm.addEventListener('submit', (e) => {
            e.preventDefault();
            submitBtn.disabled = true;
            submitBtn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Recording...';

            const fd = new FormData(donForm);

            fetch('api.php?action=add_donation', {
                method: 'POST',
                body: fd   // FormData handles multipart/form-data automatically
            })
            .then(r => r.json())
            .then(res => {
                if (res.status === 'success') {
                    ...
                    showToast(res.message || 'Failed to record donation.', 'error');
                }
            })
            .catch(() => showToast('Server error. Please try again.', 'error'))
            .finally(() => {
                submitBtn.disabled = false;
                submitBtn.innerHTML = 'Record Donation';
            });
        });
    }

...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'add_booth') {
    ...
    if (empty($name) || empty($location) || empty($state) || empty($channel)) {
        ...
        try {
            ...
            $stmt = $pdo->prepare("INSERT INTO booths (name, branch_name, location, state, channel, status) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->execute([$name, $branch_name ?: null, $location, $state, $channel, $status]);
    ...
    <div id="add-booth-form-body" style="display: none;">
        <form method="POST" action="index.php?page=my_booth" id="form-add-booth" style="padding: 1.5rem;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                ...
                <div class="form-group">
                    <label class="form-label" for="booth_branch_name">
                        <i class="fa-solid fa-building" style="margin-right: 0.3rem; color: var(--primary-blue);"></i>
                        Branch Name
                    </label>
                    <input type="text" name="branch_name" id="booth_branch_name" class="form-input"
                           placeholder="e.g. KL HQ" required>
                </div>
                <?php else: ?>
                    <div class="form-group">
            ...
            <tbody id="my-booth-tbody">
                <?php if (count($booths) === 0): ?>
                    <tr id="booth-empty-row">
                        <td colspan="<?php echo $role === 'wakalah_corporate' ? 8 : 7; ?>">
                    ...
                    <?php foreach ($booths as $index => $booth): ?>
                        <tr id="booth-row-<?php echo $booth['id']; ?>">
                            ...
                            <td style="color: var(--medium-neutral);">
                                <?php echo htmlspecialchars($booth['branch_name'] ?? '-'); ?>
                            </td>
                            <?php endif; ?>
                            <td style="color: var(--medium-neutral);">

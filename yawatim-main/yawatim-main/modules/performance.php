...
try {
    ...
    if ($role === 'admin' || $role === 'wakalah_individual') {
        ...
        $individual_rankings = $pdo->query("
            SELECT 
                ...
                w.state, 
                w.status,
                (SELECT COALESCE(SUM(amount), 0.0) FROM donations WHERE wakalah_id = w.id) as total_collections
            FROM wakalah w
    ...
    if ($role === 'wakalah_corporate' || $role === 'wakalah_individual') {
        $ch_filter = $user_channel ? "AND channel = " . $pdo->quote($user_channel) : "";
                    ...
                    <tbody>
                        <?php foreach ($individual_rankings as $index => $rank): ?>
                            <tr>
                                ...
                                <td><?php echo htmlspecialchars($rank['state']); ?></td>                                <td>

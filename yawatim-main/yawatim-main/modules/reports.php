...
if (!empty($partner_name)) {
    $partner_filter_don = " AND d.wakalah_id IN (SELECT id FROM wakalah WHERE name LIKE :partner_name)";
...
if (!empty($partner_name)) {
    $partner_filter_don_noalias = " AND wakalah_id IN (SELECT id FROM wakalah WHERE name LIKE :partner_name)";
}
...
try {
    if ($report_type === 'donations') {
        ...
        $stmt_sum = $pdo->prepare("SELECT COUNT(id) as count, COALESCE(SUM(amount), 0.0) as total, COALESCE(AVG(amount), 0.0) as average FROM donations WHERE 1=1" . $date_filter_don . $month_filter_don . $partner_filter_don_noalias . $state_filter_don);
        ...
        $sql = "
            SELECT 
                ...
                w.state, 
                w.status,
                (SELECT COALESCE(SUM(amount), 0.0) FROM donations WHERE wakalah_id = w.id " . $date_filter_don . $month_filter_don . $state_filter_don . ") as total_collections
            FROM wakalah w

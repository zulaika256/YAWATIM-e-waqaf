<?php
// db.php - MySQL database connection and optional auto-init for YAWATIM system

$DB_HOST = getenv('DB_HOST') ?: '127.0.0.1';
$DB_NAME = getenv('DB_NAME') ?: 'yawatim_db';
...
function init_db($pdo) {
    ...
    try {
        $has_wakalah = $pdo->query("SHOW TABLES LIKE 'wakalah'")->fetchColumn();
        if ($has_wakalah) {
        ...
        $has_donations = $pdo->query("SHOW TABLES LIKE 'donations'")->fetchColumn();
        if ($has_donations) {
            $columns_don = $pdo->query("SHOW COLUMNS FROM donations LIKE 'address'")->fetchAll();
            if (!empty($columns_don)) {
...
function seed_data($pdo) {
    ...
    $stmt_donation = $pdo->prepare("INSERT INTO donations (donor_name, donor_phone, donor_email, amount, donation_date, donation_month, channel, state, location, wakalah_id, booth_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

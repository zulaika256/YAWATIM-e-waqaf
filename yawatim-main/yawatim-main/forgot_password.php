...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['step'] ?? '') === 'verify_email') {
    ...
    if (empty($email) || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        $error_msg = 'Please enter your IC Number or SSM Number.';
        $step = 1;
    } else {
        try {
            $stmt = $pdo->prepare('
                SELECT u.id, u.role, w.ic_number, w.ssm_number 
                FROM users u 
                LEFT JOIN wakalah w ON u.wakalah_id = w.id 
                WHERE u.email = ?
            ');
            $stmt->execute([$email]);
            $user = $stmt->fetch();
            ...
            if ($user) {
                ...
                if ($valid) {
                    ...
                    $error_msg = 'Verification failed. IC/SSM number does not match.';
                    $step = 1;
                }
            } else {

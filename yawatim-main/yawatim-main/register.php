...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $state = ($type === 'corporate') ? ($_POST['state'] ?? '') : '';

    if ($type === 'corporate' && empty($state)) {
        ...
        if ($type === 'individual') {
            ...
            if (empty($name) || empty($ic_number) || empty($email) || empty($phone)) {
                ...
                try {
                    ...
                    if ($stmt->fetchColumn() > 0) {
                        ...
                        if ($type === 'individual') {
                            ...
                            $stmt_wak->execute([$name, $email, $phone, $ic_number]);
                            ...
                            $stmt_user->execute([$email, $pass_hash, $wak_id]);
                        } else {
                            ...
                            $stmt_wak->execute([$name, $email, $phone, $state, $representative, $ssm_number, $hq_address]);
                            $wak_id = $pdo->lastInsertId();

                            // Insert into users
                            $pass_hash = password_hash($password, PASSWORD_DEFAULT);
                            $stmt_user = $pdo->prepare("INSERT INTO users (email, password_hash, role, state, wakalah_id) VALUES (?, ?, 'wakalah_corporate', ?, ?)");
                            $stmt_user->execute([$email, $pass_hash, $state, $wak_id]);
                        }
